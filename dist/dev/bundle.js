...
/***/ function(module, exports, __webpack_require__) {
	...
	var _scrollyMap = __webpack_require__(4);

	var _scrollyMap2 = _interopRequireDefault(_scrollyMap);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var scrollyMap = function scrollyMap() {
	    ...
	    var shootingA = new CountUp('shootingCount', 0, 94, 0, 2, option);
	    var shootingB = new CountUp('killCount', 0, 790, 0, 4, option);
	    var shootingC = new CountUp('injureCount', 0, 1983, 0, 6, option);
	    shootingA.start();
	    shootingB.start();
	    shootingC.start();
	    ...
	    var lightBackground = "https://api.mapbox.com/styles/v1/gabriel-florit/cj8nbv8ow7m9g2rntzv29ckj5/tiles/256/{z}/{x}/{y}?access_token=pk.eyJ1IjoiZ2FicmllbC1mbG9yaXQiLCJhIjoiVldqX21RVSJ9.Udl7GDHMsMh8EcMpxIr2gA";
	    var blackBackground = "https://api.mapbox.com/styles/v1/gabriel-florit/cj8vp0rr7fe232rk5mg0vdwf5/tiles/256/{z}/{x}/{y}?access_token=pk.eyJ1IjoiZ2FicmllbC1mbG9yaXQiLCJhIjoiVldqX21RVSJ9.Udl7GDHMsMh8EcMpxIr2gA";
	    var satellite = "https://api.mapbox.com/styles/v1/mapbox/satellite-v9/tiles/256/{z}/{x}/{y}?access_token=pk.eyJ1IjoiaGVybWlvbmV3eSIsImEiOiJjaXZ5OWI1MTYwMXkzMzFwYzNldTl0cXRoIn0.Uxs4L2MP0f58y5U-UqdWrQ";
	    //map layer
	    var streetMap = L.tileLayer(blackBackground, {
	        id: 'mapbox.street',
	    ...
	    d3.csv('./assets/usMassShootingfromMotherJones2017_revised.csv', function (data) {
	        ...
	        info.update = function (s) {
	            ...
	            this._div.innerHTML = s ? '<h4>' + s['Case'] + '</h4>' + '<p><span>Date: </span>' + dateFormat(s['Date']) + '<br/>' + '<span>Fatalities: </span> ' + s['Fatalities'] + '<br/>' + '<span>Details: </span> ' + s['Summary'] + '<br/>' + (s['Weapons obtained legally'] == 'No' ? '<span>Weapons:</span> Illegal <br/>' : '') + (s['Prior signs of mental health issues'] == 'Yes' ? '<span>Shooter:</span> Had shown signs of mental health issues. </p>' : '</p>')
	            //'<b>' + h.name + '</b><br />' +
	            : 'Hover over a circle';
	        };
	        info.addTo(map);
	        ...
	        function animationCircles() {
	            ...
	            circlesByYear = setInterval(function () {
	                ...
	                if (counter < 34) {
	                    //console.log(counter);
	                    nestedData[counter].values.forEach(function (shooting) {
	    ...
	    var scene0 = new ScrollMagic.Scene({ triggerElement: "#spacer0", duration: 100, triggerHook: 0, reverse: true }).setPin("#map");

	    var scene = new ScrollMagic.Scene({ triggerElement: "#trigger0", duration: 400, triggerHook: 0, reverse: true }).setPin("#map").on('start', function () {
	        map.flyTo([40.104, -97.485], 4);
	    });

	    var sceneA = new ScrollMagic.Scene({ triggerElement: "#trigger1", duration: 400, triggerHook: 0, reverse: true }).setPin("#map").on('start', function () {
	    ...
	    function dateFormat(s) {
	        var year = s.split('/')[2] > 20 ? s.split('/')[2] : +s.split('/')[2] + 2000,
	            day = s.split('/')[1],
	            month = s.split('/')[0] - 1;
	        var newDate = new Date(year, month, day).toString();
